    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div class="bg-darkbg-900/60 backdrop-blur-md border border-zinc-900 p-5 rounded-2xl flex items-center justify-between shadow-xl">
            <div class="space-y-1">
                ...
                <h3 class="text-2xl font-black text-emerald-400">Rp {{ number_format($total_pemasukan, 2, ',', '.') }}</h3>
        ...
        <div class="bg-darkbg-900/60 backdrop-blur-md border border-zinc-900 p-5 rounded-2xl flex items-center justify-between shadow-xl">
            <div class="space-y-1">
                ...
                <h3 class="text-2xl font-black text-red-400">Rp {{ number_format($total_pengeluaran, 2, ',', '.') }}</h3>
        ...
        <!-- Saldo Bersih (Premium Glow Card) -->
        <div class="bg-zinc-950/90 border border-brand-500/30 p-5 rounded-2xl flex items-center justify-between shadow-2xl relative overflow-hidden">
            ...
            <div class="space-y-1 z-10">
                ...
                <h3 class="text-2xl font-black text-white">Rp {{ number_format($saldo, 2, ',', '.') }}</h3>
                    ...
                    @forelse($transactions as $trans)
                        <tr class="hover:bg-zinc-900/10 transition-colors">
                            <td class="px-6 py-4 text-xs font-semibold text-zinc-400">
                                ...
                                <span>{{ date('d M Y', strtotime($trans->date)) }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <h4 class="font-bold text-zinc-200 text-sm">{{ $trans->category }}</h4>
                                <p class="text-xs text-zinc-500 mt-1 max-w-xs truncate" title="{{ $trans->description }}">{{ $trans->description ?? '-' }}</p>
                            </td>
                            <td class="px-6 py-4 text-right font-black text-sm {{ $trans->type == 'Pemasukan' ? 'text-emerald-400' : 'text-red-400' }}">
                                {{ $trans->type == 'Pemasukan' ? '+' : '-' }} Rp {{ number_format($trans->amount, 2, ',', '.') }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="text-[10px] px-2.5 py-0.5 rounded-full font-semibold {{ $trans->type == 'Pemasukan' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20' }}">
                                    {{ $trans->type }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-xs text-zinc-400 font-medium">
                                {{ $trans->user ? $trans->user->name : 'System' }}
                            </td>
                            <td class="px-6 py-4 text-right">
                                <form action="{{ route('keuangans.destroy', $trans->id) }}" method="POST" onsubmit="return confirm('Hapus transaksi kas ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-8 h-8 rounded-lg bg-zinc-950 border border-zinc-850 flex items-center justify-center text-zinc-500 hover:text-red-400 hover:border-red-500/30 transition-colors" title="Hapus Transaksi">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-zinc-500">
                                <i class="fa-solid fa-money-bill-transfer text-3xl text-zinc-800 mb-2.5 block"></i>
