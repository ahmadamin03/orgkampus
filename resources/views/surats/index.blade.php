...
<div class="bg-darkbg-900/60 backdrop-blur-md border border-zinc-900 rounded-2xl shadow-xl">
    ...
    <div class="p-6 border-b border-zinc-900 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-zinc-500 mt-1">Total arsip: {{ $surats ? $surats->total() : 0 }} dokumen terdaftar</p>
        </div>
    ...
    <div class="p-4 border-t border-zinc-900 flex items-center justify-between">
        ...
        <div class="flex gap-2">
            ...
            @else
                <a href="{{ $surats->previousPageUrl() }}" class="px-3 py-1.5 rounded-lg bg-zinc-950 border border-zinc-800 text-zinc-400 hover:text-white hover:border-zinc-700 text-xs font-medium transition-colors">Sebelumnya</a>
            @endif
            @if ($surats->hasMorePages())
                <a href="{{ $surats->nextPageUrl() }}" class="px-3 py-1.5 rounded-lg bg-zinc-950 border border-zinc-800 text-zinc-400 hover:text-white hover:border-zinc-700 text-xs font-medium transition-colors">Selanjutnya</a>
            @else
